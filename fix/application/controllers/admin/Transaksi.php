...
class Transaksi extends CI_Controller {
    ...
    public function pengiriman()
    {
         ...
         $data = array(  'title'             => 'Data Transaksi',
                         'transaksi'         =>  $transaksi,
                         'isi'               => 'admin/transaksi/pengiriman'
                          );
         $this->load->view('admin/layout/wrapper', $data, FALSE);
    }
   ...
   public function editpengiriman($kode_transaksi)
    {
        //ambil data yang akan diedit
        $transaksi = $this->transaksi_model->detailpengiriman($kode_transaksi);
        //ambil data kategori
        ...
        $valid->set_rules('nama_penerima','Nama penerima','required', 
                array( 'required'    =>'%s harus diisi'));
        ...
        $data = array('title'    => 'Edit Data Pengiriman: '.$transaksi->nama_pelanggan,
                    'transaksi'         => $transaksi,
                    
                    'error'    => $this->upload->display_errors(),
                    'isi'      => 'admin/transaksi/editpengiriman'
                    );
        $this->load->view('admin/layout/wrapper', $data, FALSE);
        //masuk databese
        }else{
            ...
            $i = $this->input;
            //slug
            $slug_transaksi = url_title($this->input->post('nama_penerima').'-'.$this->input->post('no_resi'),'dash',TRUE  );
            $data = array(  

                'id_user'             =>  $this->session->userdata('id_user'),
              
                'kode_transaksi'      =>  $i->post('kode_transaksiHidden'),
              
             
                // 'nomor_spk'             =>  $i->post('nomor_spk'),
                // 'nama_pelanggan'        =>  $i->post('nama_pelanggan'),
                // 'nama_produk'           =>  $i->post('nama_produk'),
                ...
                'tgl_diterima'          =>  $i->post('tgl_diterima'),
                'keterangan'            =>  $i->post('keterangan'),
                'bukti_penerimaan'      =>  $upload_gambar['upload_data']['file_name']
               


                );
            $this->transaksi_model->editpengiriman($data);
            $this->session->set_flashdata('sukses', 'Data pengiriman telah diedit');
           redirect(base_url('admin/transaksi/pengiriman'),'refresh');

             }}else{
            //edit pengiriman tanpa edit gambar
            $i = $this->input;
            //slug
           $slug_transaksi = url_title($this->input->post('nama_penerima').'-'.$this->input->post('no_resi'),'dash',TRUE  );
            $data = array(  
      ...
      //Detail Pengiriman
      public function detailpengiriman($kode_transaksi)
       {
        ...
        $data = array('title'        => 'Data Pengiriman',
                      ...
                      'isi'          => 'admin/transaksi/detailpengiriman'
                     );
                $this->load->view('admin/layout/wrapper', $data, FALSE);
        }
